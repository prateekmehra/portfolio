function RadarChart() {
        ...
        selection.each(function () {
            ...
            update = function() {
                ...
                var update_axis_legends = update_axes.selectAll("." + options.class + "AxisLegend")
                    .data(function(d) { return [d]; }, get_axis_label)
                ...
                update_blobWrapper.enter()
                   ...
                   .attr("id" , function(d) { return 'RW-' + d.key.split(/[^A-Za-z0-9]/g).join('-'); })
                  .on("mouseover", function(d,i){
                            $("g#L-" + keyScale(i).split(/[^A-Za-z0-9]/g).join('-')).toggleClass('hover');
                            // $("g.cell#L-" + keyScale(i).split(/[^A-Za-z0-9]/g).join('-'))[0].addClass('hover');


                            //$("g.cell#L-" + keyScale(i).split(/[^A-Za-z0-9]/g).join('-')).trigger('mouseover');

                            var flip = JSON.parse(JSON.stringify(d));
                            flip.values.map(function(i){ flip[i.axis] = i.original_value; });
                            graph.highlight([flip]);
                            
                            var _grid = 'div#grid-'+keyScale(i).split(/[^A-Za-z0-9]/g).join('-');
                            d3.select(_grid)
                            .classed('hover', true)
                            .style("display","block");
                        })
                        .on("mouseout", function(d,i){
                            graph.unhighlight(graph.selection);
                            var _grid = 'div#grid-'+keyScale(i).split(/[^A-Za-z0-9]/g).join('-');
                            d3.select(_grid)
                            .classed('hover', false)
                            .style("display","none");
                ...
                update_radarArea.enter()
                   .append("path")
                   .attr("class", function(d) { return options.class + "RadarArea " + d.key.replace(/[^A-Za-z0-9]/g, '-') })
                ...
                if (options.legend.display) {
                   ...
                   if (d3.legend) {
                      ...
                      legend_node.selectAll('.cell')
                        .attr('id', function(d, i) { return keyScale(i) && 'L-' + keyScale(i).split(/[^A-Za-z0-9]/g).join('-') })
                        ...
                        .on("mouseover", function(d,i){
                            var flip = JSON.parse(JSON.stringify(_data[d]));
                            flip.values.map(function(i){ flip[i.axis] = i.original_value; });
                            graph.highlight([flip]);
   ...
   function areaMouseover(d, i, self) {
      ...
      d3.select(self)
         ...
         .style("display", function(d, i, j) {
            return options.areas.filter.indexOf(d.key) >= 0 ? "none" : "block";
         })
			.style("fill-opacity", function(d, i, j) {
            return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.95;
         });	
   }
   ...
   // on mouseover for the legend symbol
	function legendMouseover(d, i, self) {
         ...
         var area = keys.indexOf(d) >= 0 ? d : keyScale(d); 
			...
			//Bring back the hovered over blob
         chart_node.selectAll("." + options.class + "RadarArea." + area.split(/[^A-Za-z0-9]/g).join('-'))
				.transition().duration(200)
				.style("fill-opacity", function(d, i, j) {
               return options.areas.filter.indexOf(d.key) >= 0 ? 0 : 0.95;
            });	
   ...
   var get_axis_label = function(d) { return d && getLabel(d.axis); };
